...
static XDKAirMenuController *controller;
...
+ (id) sharedInstance {
    @synchronized ([XDKAirMenuController class]) {
        if (controller == nil) {
            controller = [[XDKAirMenuController alloc] init];
        }
    }
...
{
    [self.tableView cellForRowAtIndexPath:lastIndexPath].backgroundColor = [UIColor clearColor];
    lastIndexPath = indexPath;
    [self.tableView cellForRowAtIndexPath:indexPath].backgroundColor = [UIColor colorWithRed:50/255.0 green:60/255.0 blue:90/255.0 alpha:1];
